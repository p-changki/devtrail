...
#
# 역할 분리:
#   로컬(여기)  빠른 1차 게이트. 내 작업 트리를 본다.
#   GitHub CI   독립적인 최종 검증. 깨끗한 체크아웃 + ubuntu 로케일.
#               로컬이 통과했다고 CI 를 건너뛰지 않는다.
set -u
...
if [ "$FAIL" = 0 ]; then
  ...
  echo "   ⚠️ GitHub CI 는 여전히 독립적으로 재검증합니다. 로컬 통과가"
  echo "      CI 통과를 보장하지 않습니다 (깨끗한 체크아웃 · ubuntu 로케일)."
  exit 0
fi
