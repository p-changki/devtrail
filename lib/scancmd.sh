...
scan_cmd() {
  ...
  if [ -z "$vault" ]; then
    config_exists || die "볼트 경로를 알 수 없습니다.
   devtrail scan /경로/볼트   처럼 직접 지정하거나 'devtrail init' 을 먼저 실행하세요."
    vault="$(vault_path)"
  fi
  [ -d "$vault" ] || die "볼트 경로 없음: $vault"
  [ -f "$DT_SCAN_PY" ] || die "진단기 없음: $DT_SCAN_PY"
...
_scan_render() {
  ...
  _d_section "볼트"
  info "  경로   $(jq -r '.vault' "$j")"
  info "  노트   ${notes}개 · 폴더 $(jq -r '.scale.folders' "$j")개"
  info "  메타   frontmatter ${fm_pct}%"
...
_scan_fields() {
  local j="$1"
  _d_section "메타데이터 커버리지"
  dim "   '키만 있는 것'과 '값까지 있는 것'을 구분합니다 — 쿼리는 값이 있어야 동작합니다."
  jq -r '
  ...
  ' "$j" | while IFS=$'\t' read -r k wk kp wv vp; do
    ...
    printf '  %s %-12s 키 %4s개 (%4s%%)   값 %4s개 (%4s%%)\n' "$mark" "$k" "$wk" "$kp" "$wv" "$vp"
  done

  local custom; custom=$(jq -r '[.custom_fields[] | .[0]] | join(" · ")' "$j")
  [ -n "$custom" ] && [ "$custom" != "" ] && dim "   그 밖의 필드: $custom"
}
...
_scan_tags() {
  ...
  _d_section "태그"
  if [ "$total" = "0" ]; then
    warn "태그를 쓰지 않는 볼트입니다 — 자동 라우팅이 동작하지 않습니다"
    dim "   'devtrail adopt' 로 폴더 매핑만 쓰거나, 'devtrail align' 으로 태그를 채웁니다."
    return
  fi
  info "  총 ${total}회 사용 · #type/* 네임스페이스 ${tpct}%"
  jq -r '.tags.top[0:8][] | "     \(.[1])회  #\(.[0])"' "$j"
  if awk "BEGIN{exit !($tpct < 30)}"; then
    warn "  #type/* 사용률이 낮아 태그 기반 집계가 대부분 빈 결과가 됩니다"
    dim "   scan 이 뽑은 상위 태그를 우리 type 에 매핑하면 노트를 고치지 않고 쓸 수 있습니다."
  fi
}
...
_scan_roles() {
  ...
  _d_section "폴더 역할 추론"
  if [ "$n" = "0" ]; then
    dim "   역할로 보이는 폴더를 찾지 못했습니다 (빈 볼트이거나 구조가 다릅니다)"
    return
  fi
  dim "   폴더 '이름'이 아니라 내용의 형태로 추론합니다. 확정은 사용자가 합니다."
  jq -r '
...
_scan_obsidian() {
  ...
  if [ "$(jq -r '.obsidian.present' "$j")" != "true" ]; then
    warn "Obsidian 설정 폴더(.obsidian)가 없습니다"
    dim "   볼트를 Obsidian 에서 한 번 열어야 플러그인 설정을 병합할 수 있습니다."
    return
  fi
  ...
  [ -n "$miss" ] && fail "필수 플러그인 누락: $miss" || ok "필수 플러그인 4/4"
  [ -n "$coremiss" ] && warn "코어 플러그인 꺼짐: $coremiss"
  [ -n "$rec" ] && dim "   권장 미설치: $rec"
  [ "$(jq -r '.obsidian.plugins.zk_prefixer_on' "$j")" = "true" ] \
    && warn "zk-prefixer 가 켜져 있습니다 — Zettel ID 가 이중으로 붙습니다"
  ...
  [ "$hk" != "0" ] && {
    warn "단축키 ${hk}개가 이미 쓰이는 중 — 빈 키로 재배정합니다"
    jq -r '.obsidian.conflicts.hotkeys[0:4][] | "       \(.combo) ← \(.taken_by)"' "$j"
  }
  [ "$fold" != "0" ] && warn "만들려는 폴더 ${fold}개가 이미 존재합니다 (기존 것을 그대로 씁니다)"
  [ "$anm" != "0" ] && {
    warn "Auto Note Mover 규칙이 이미 ${anm}개 있습니다 (트리거: ${trig})"
    [ "$trig" = "Automatic" ] && \
      dim "   ⚠️ 자동 이동이 켜져 있습니다. 기존 모드는 Manual 로 시작해 기존 노트를 지킵니다."
  }
  [ "$(jq -r '.obsidian.conflicts.linter_present' "$j")" = "true" ] && \
    dim "   Linter 설정이 있습니다 — 기존 모드에서는 건드리지 않습니다(저장 시 서식이 바뀌지 않게)"
  ...
  if [ "$rag" = "true" ]; then
    ok "RAG (Smart Connections) 사용 중 · 인덱스 $(jq -r '(.obsidian.rag.index_bytes/1048576|floor)' "$j")MB"
    [ "$(jq -r '.obsidian.rag.excluded_configured' "$j")" != "true" ] && \
      dim "   제외 폴더가 설정되지 않았습니다 — 자동 수집물이 인덱스를 오염시킵니다"
  else
    dim "   RAG 미사용 (선택 기능)"
  fi
}
...
_scan_mode() {
  ...
  if [ "$notes" -lt 10 ] && [ "$has_dot" != "true" ]; then
    ok "새로 시작하기 — 빈 볼트입니다"
    dim "   전체 구조를 만들고 설정을 전부 적용합니다."
  elif [ "$notes" -lt 10 ]; then
    ok "새로 시작하기 — 노트가 거의 없습니다"
  else
    ok "기존 볼트에 얹기 — 노트 ${notes}개가 있습니다"
    dim "   기존 폴더를 그대로 쓰고 설정만 매핑합니다. 노트를 움직이지 않습니다."
    dim "   더 안전하게 하려면 '분리 설치' — 새 하위 트리에만 넣고, 지울 땐 폴더째 지웁니다."
  fi
  echo
  dim "   적용: devtrail init   (이 진단 결과로 기본값을 제안합니다)"
}
