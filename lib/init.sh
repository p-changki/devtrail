...
init_run() {
  ...
  root=$(_init_ask "볼트 안 최상위 폴더명" "notes")
    ...
    -f /dev/stdin > "$CONFIG_FILE" <<'JQ'
{
  ...
  dirs:    { devlog: "devlog", weekly: "weekly", templates: "templates",
             projects: "projects", repodocs: "repodocs" },
...
_init_next_steps() {
  ...
  info "  2) 플러그인 3개를 설치·활성화하고 Obsidian을 재시작합니다"
  dim "     Shell commands · Templater · Dataview"
