...
const raw = (await tp.system.prompt("Library name"))?.trim();
if (!raw) { dtCancel(app.workspace.getActiveFile()); tR = ""; return; }
const desc = ((await tp.system.prompt("One-line description (goes in the filename)", "")) || "").trim();
const cat = ((await tp.system.prompt("Category (ui/state/form/chart/auth …)", "general")) || "general").trim();
// The description goes in the filename so the list alone tells you what it was.
