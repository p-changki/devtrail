...
const raw = (await tp.system.prompt("Reference card title"))?.trim();
if (!raw) { dtCancel(app.workspace.getActiveFile()); tR = ""; return; }

const labels = ["Document", "Certificate", "Contract", "Receipt", "Identity", "Other"];
const values = ["document", "certificate", "contract", "receipt", "identity", "misc"];
const kind = await tp.system.suggester(labels, values, false, "Kind") || "misc";
const source = ((await tp.system.prompt("Source / issuer (optional)", "")) || "").trim();
