...
#### 재검토 1 — 2026-08-22, 1500줄을 넘었다

디자인 핸드오프 반영과 P0 수정으로 `plugin/main.js` 가 **1664줄**이 됐다
(재검토 시점 1543 → 빠른 기록 모달·최근 기록 전체 화면이 더해졌다).
D3 이 정한 뒤집는 조건이 참이 됐으므로 다시 본다.

**살펴본 것**

| | |
|---|---|
| 죽은 코드 | `projectRow` 하나를 걷어 18줄 줄었다. 더 없다 |
| 무엇이 큰가 | 프로젝트 보드 320 · 홈 315 · 리뷰 169 · 재설계 데이터 155 |
| 중복인가 | 아니다. 홈의 표(핸드오프)와 프로젝트 탭의 칸반은 다른 화면이다 |

**세 갈래**

1. **빌드 도구 도입** — D3 의 원래 근거가 아직 유효하다. 기여자는 한 명이고,
   런타임 오류는 테스트가 잡고 있다(이번 세션에만 변이로 확인한 것이 40회가
   넘는다). 세 번째 툴체인의 비용이 여전히 더 크다.

2. **파일 분리** — Obsidian 은 `main.js` 하나만 읽는다. CommonJS `require`
   로 형제 파일을 부르는 플러그인이 있지만 공식 계약이 아니고, **이 자리에서
   확인할 방법이 없다.** 틀리면 플러그인이 통째로 안 뜬다. 검증 없이 걸 수 없다.

3. **한 파일을 유지하되 경계를 만든다** ← 채택

**결정**

한 파일을 유지한다. 대신 **분리 가능한 경계**를 코드에 명시하고 테스트가
그 순서를 지키게 한다:

```
읽기 모델      vault 를 읽어 값으로 바꾼다        (순수, DOM 없음)
명령 탐색      레지스트리에서 부를 것을 고른다    (순수)
화면 렌더링    값을 DOM 으로                      (Obsidian API)
쓰기 동작      아직 없다                          (칸반 상태 변경 PR 에서)
```

**언제 쪼개나.** 칸반의 상태 변경(쓰기)이 들어올 때다. 그때 네 번째 관심사가
실제로 생기고, 그 PR 이 배포 목록(artifact manifest)까지 함께 다룬다 —
파일이 늘면 설치·업데이트·undo 가 새 파일을 알아야 한다.

⚠️ 그때까지 이 재검토 기록이 게이트다. 1500 을 넘은 채로 **기록 없이**
   지나가지 못한다.
