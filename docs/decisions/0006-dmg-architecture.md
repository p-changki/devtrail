...
그럼 왜 지금 만드나 — **구조는 지금 굳혀야** 한다. Developer ID 가 생겼을 때
바꾸는 것은 서명 한 줄이지 구조가 아니다. 구조를 미루면 공증 실패와 구조
결함이 같은 날 한꺼번에 온다.
...
- 온보딩 UI (M4 4b)
- 릴리즈 매니페스트에 DMG 해시 기록 — 지금 넣으면 순서가 순환한다
  (매니페스트 → DMG → 매니페스트). 릴리즈 단계(M7·M8)에서 정한다.
