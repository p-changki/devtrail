...
**플러그인은 다른 한도를 쓴다 — `plugin/*.js` 합계 1500줄에서 재검토.**
Obsidian 플러그인은 `main.js` 가 진입점이라 셸 라이브러리와 같은 기준으로
자를 수 없다. 1500 은 상한이 아니라 **다시 보는 조건**이다. 하드 캡으로
바꾸면 정직하게 늘어난 코드를 막고, 그러면 사람은 테스트를 지우거나 숫자를
올린다. 넘는 것이 아니라 **기록 없이 넘는 것**을 막는다.
`tests/test-command-center.sh` 가 아래 기록을 읽는다.

#### 재검토 1 — 2026-08-27 · 합계 2514줄

조건이 참이 됐으므로 다시 봤다.

| | |
|---|---|
| 무엇이 큰가 | `view.js` 1227 · `read-model.js` 449 · `main.js` 419 · `i18n.js` 225 · `commands.js` 194 |
| 빌드 도구 | 도입하지 않는다. CI 는 bash·python·swift 세 갈래이고 Node 는 네 번째 툴체인이 된다 |
| 파일 분리 | 이미 했다. `main.js` 가 `${dir}/*.js` 를 require 하고, 배포 목록은 `plugin/files.json` 하나가 정한다 |

**결정** — 지금 구조를 유지한다. 합계가 는 것은 화면이 늘어서지 한 파일이
두 가지를 해서가 아니다. 다음 경계는 `view.js` 다 — 혼자 절반을 차지하므로,
다음 재검토에서 화면 단위로 쪼갤 수 있는지 본다.

#### 재검토 2 — 2026-08-27 · `view.js` 1227줄을 쪼갤 것인가

재검토 1 이 예고한 대로 `view.js` 를 봤다. **쪼개지 않는다.**

**먼저 확인한 것 — 어떤 게이트도 이것을 요구하지 않는다.** 400/600 줄 규칙
(위 §10)이 도는 대상은 `lib/*.sh` · `lib/merge/*.sh` · `lib/init/*.sh` ·
`lib/gen/*.py` · `bin/devtrail` 이고 `plugin/*.js` 는 들어 있지 않다. 1500 은
**합계**를 보므로 쪼개도 숫자가 줄지 않는다 — 애초에 쪼개서 한도를 피하는
것을 막으려고 합계로 세기 때문이다. 즉 이 검토는 게이트 대응이 아니라 순수한
유지보수 판단이다.

**결합도는 낮다.** 탭 렌더러 830 줄이 참조하는 `this` 멤버는 18 종뿐이다 —
상태 9(`app` 24회 · `render` 8 · `paths` 8 · `route` 5 · `files` 3 ·
`recentShown` 3 · 라이브러리 필터 2+2 · `lang` 1), 공용 primitive 4, 서로
부르는 렌더러 5. `this.` 를 `view.` 로 바꾸면 순수 함수가 된다.

| 경계 후보 | 줄 | `this.` |
|---|---|---|
| `QuickCaptureModal` | 85 | 24 (자기 인스턴스) |
| 공용 primitives (`action`·`card`·`badge`·`row`) | 57 | 3 (전부 `this.app`) |
| 프로젝트 탭 | 135 | 7 |
| 자료 탭 | 149 | 16 |
| 홈 패널 | 346 | 18 |

**그런데 비용이 실익보다 크다.** 모듈끼리 상대 `require` 를 하지 않기로 한
결정(ADR 0004) 때문에 로딩은 `main.js` 한 곳이고, 필요한 것은 인자로 넘긴다.
`makeView` 는 이미 40 여 개를 구조분해로 받는데, 하위 렌더러로 쪼개면 그것을
**다시 넘기는 배선**이 생긴다. 복잡도가 줄지 않고 옮겨 간다. 파일 하나마다
`plugin/files.json` · `plugin/main.js` · `tests/lib/render-view.js` 를 함께
고쳐야 한다.

**다음 재검토 조건** — 아래 중 하나가 참이 되면 다시 본다. "다음에 본다" 만
남기면 다음 사람이 같은 검토를 처음부터 반복한다.

- 기여자가 둘 이상이 된다
- **한 탭을 고치다 다른 탭을 깨뜨리는 일이 실제로 발생한다** ← 가장 믿을 만한 신호
- `view.js` 가 **혼자** 1500 줄을 넘는다
...
| `preset/templates/` | Obsidian 노트 템플릿 23종 + 공용 JS 헤더 |
...
- [command-center-usage.md](./command-center-usage.md) — Command Center 사용 가이드

구현계획 · ADR · 리서치 · 디자인 토큰은 공개 저장소에 두지 않는다
(`.gitignore`). 그중 **게이트가 읽어야 하는 결정**은 이 문서에 남긴다 —
추적하지 않는 파일을 검사가 읽으면 그 검사는 조용히 죽는다.
