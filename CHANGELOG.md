...
- **아무것도 넣지 않고 초록 카드를 띄웠다.** `activity.sh` 는 헤딩이나
  개발일지를 못 찾으면 안내를 stderr 로 찍고 **exit 0** 으로 끝난다 —
  Obsidian 파일생성 이벤트에서 남의 노트마다 에러를 내지 않으려는 의도적
  선택이다. 앱은 그 종료코드만 보고 stdout 이 비었다는 이유로 성공 문구를
  채웠다. 같은 화면의 헤더는 "아직 삽입 전" 이라고 말하고 있었다 — 한 화면이
  서로 다른 말을 했다.

  이제 stdout 의 `활동 삽입 완료` · `이미 삽입됨` 표식으로만 판정하고, 없으면
  stderr 안내를 그대로 빨간 카드로 보여준다. 사용자가 다시 누를 수 있다.

- **`devtrail doctor` 가 개발일지 템플릿의 헤딩을 본다.** 기존 검사는 이미
  만들어진 일지만 봐서, 헤딩이 깨진 템플릿으로 만든 일지가 나오기 전까지
  통과했다.

### 바뀐 것

- ⚠️ **`devtrail template update --apply` 가 확인을 받는다.** 새 템플릿에
  설정된 헤딩(`headings.*`)이 없으면 무엇이 안 되는지 말하고 confirm 을
  기다린다. 대답을 못 받으면 덮어쓰지 않는다.

  헤딩은 두 곳에 있다 — 노트를 만드는 **볼트 템플릿**과, 그 노트에서 넣을
  자리를 찾는 **`headings.*`**. 둘은 짝이고, `template update --apply` 가
  그중 한쪽만 조용히 바꿔 짝을 깼다. 깨진 사실은 다음 날 만든 일지에서야
  드러났다.

  > **비대화형으로 이 명령을 부르던 스크립트는 헤딩이 맞지 않을 때 멈춘다.**
  > 이전에는 조용히 덮어썼다. 멈추는 쪽이 옳지만 동작이 달라진 지점이라
  > 적어 둔다.

### 내부

- 회귀 테스트 13건 추가 (doctor 템플릿 검사 · `template update` 게이트 ·
  활동 삽입 판정). 변이 주입 3건 전부 RED 확인.
